...
import { getSheetRows, hasGoogleConfig, updateChecklistStatus } from "@/lib/googleSheets";
...
async function checklistData() {
  try {
    ...
    console.error("Failed to build checklist payload, falling back to mock data.", error);
  }
...
export async function GET() {
  ...
  return NextResponse.json(
    { ...payload, canWrite: hasGoogleConfig() && payload.source === "google", refreshedAt: new Date().toISOString() },
    {
      headers: {
